...
@section('meta-content')
  ...
  <meta property="og:image"
        content="{{ $data->og_image ? asset('media/' . $data->og_image) : asset('media/' . $data->f_thumbnail) }}" />
        ...
        <div class="row">
          ...
          <div class="col-xl-7">
            <div class="pr_details">
              ...
              @endif
              @if ($data->shop_name != '')
                <div class="pr_extra"><strong>{{ __('Sold By') }}:</strong> <a
              ...
              <div class="product_price">
                ...
                @if ($data->is_discount == 1 && $data->old_price != '')
                  ...
                  @if ($gtext['currency_position'] == 'left')
                    <div class="old-item-price">{{ $gtext['currency_icon'] }}{{ NumberFormat($data->old_price) }}</div>
                    <span class="discount">-{{ $discount }}%</span>
                  @else
                    <div class="old-item-price">{{ NumberFormat($data->old_price) }}{{ $gtext['currency_icon'] }}</div>
                    <span class="discount">-{{ $discount }}%</span>
                  @endif
                @endif
              </div>
              ...
              <div class="pr_quantity">
                ...
                <input name="quantity" id="quantity" type="number" min="1"
                       max="{{ $data->is_stock == 1 ? $data->stock_qty : 999 }}" value="1">
              </div>
              <div class="pr_buy_cart">
                <a class="btn theme-btn cart product_addtocart" data-id="{{ $data->id }}"
                   data-stockqty="{{ $data->is_stock == 1 ? $data->stock_qty : 999 }}"
                   href="javascript:void(0);">{{ __('Add To Cart') }}</a>
                <a class="btn theme-btn cart product_buy_now" data-id="{{ $data->id }}"
                   data-stockqty="{{ $data->is_stock == 1 ? $data->stock_qty : 999 }}"
                   href="javascript:void(0);">{{ __('Buy Now') }}</a>
                <a class="btn theme-btn cart wishlist addtowishlist" data-id="{{ $data->id }}"
                   href="javascript:void(0);"><i class="bi bi-heart-fill"></i></a>
              </div>

              @if ($data->is_stock == 1)
                @if ($data->stock_status_id == 1)
                  <div class="pr_extra"><strong>{{ __('Availability') }}:</strong><span
                          class="instock">{{ $data->stock_qty }} {{ __('In Stock') }}</span></div>
                @else
                  <div class="pr_extra"><strong>{{ __('Availability') }}:</strong><span
                          class="stockout">{{ __('Out Of Stock') }}</span></div>
                @endif
              @endif
              @if ($data->is_stock == 1)
                @if ($data->sku != '')
              ...
              @endif
              @if ($data->brandname != '')
                <div class="pr_extra"><strong>{{ __('Brand') }}: </strong><a
                     href="{{ route('frontend.brand', [$data->brand_id, str_slug($data->brandname)]) }}">
                    {{ $data->brandname }}</a></div>
              @endif
              <div class="pr_extra"><strong>{{ __('Category') }}: </strong> <a
                   href="{{ route('frontend.product-category', [$data->cat_id, $data->cat_slug]) }}">{{ $data->cat_name }}</a>
              </div>

              <div class="pr_widget">
                <label class="widget-title">{{ __('Share this') }}</label>
                <div class="social-media">
                  <a href="https://www.facebook.com/sharer/sharer.php?u={{ route('frontend.product', [$data->id, $data->slug]) }}"
                     target="_blank"><i class="bi bi-facebook"></i></a>
                  <a href="https://twitter.com/intent/tweet?text={{ $data->title }}&url={{ route('frontend.product', [$data->id, $data->slug]) }}"
                     target="_blank"><i class="bi bi-twitter"></i></a>
                  <a href="http://www.linkedin.com/shareArticle?mini=true&url={{ route('frontend.product', [$data->id, $data->slug]) }}&title={{ $data->title }}&summary={{ $data->short_desc }}"
                     target="_blank"><i class="bi bi-linkedin"></i></a>
                  <a href="https://wa.me/?text={{ route('frontend.product', [$data->id, $data->slug]) }}"
                    ...
                    <div class="row">
                      <div class="col-lg-6">
                        <div class="review-form">
                          ...
                          <p>Please <a href="{{ route('frontend.login') }}"><strong>login</strong></a> to write review!
                          </p>
                          <div class="form-product-review">
                            <form class="form" method="POST" action="{{ route('frontend.saveReviews') }}">
                              ...
                              @if (isset(Auth::user()->name))
                                ...
                                <div class="mb-3">
                                  <label for="rating"
                                         class="form-label">{{ __('Your rating of this product') }}</label>
                              ...
                              @else
                                ...
                                <a class="btn theme-btn" href="{{ route('frontend.login') }}"><i
                                     class="bi bi-box-arrow-in-right"></i> {{ __('Please Login') }}</a>
                              @endif
                            </form>
                          </div>
                        </div>
                      ...
                      <div class="col-lg-12">
                        <div class="stores-card">
                          <div class="store-logo">
                            ...
                            @else
                              <img src="{{ asset('media/' . $seller_data->photo) }}"
                                   alt="{{ $seller_data->shop_name }}" />
                            @endif
                          </div>
                          <div class="desc">
                            <h3 class="store-name"><a
                                 href="{{ route('frontend.stores', [$data->seller_id, str_slug($data->shop_url)]) }}">{{ $seller_data->shop_name }}</a>
                            </h3>
                            <h6 class="since">{{ __('Since') }} {{ date('Y', strtotime($seller_data->created_at)) }}
                            </h6>
                            <div class="rating-wrap">
                              <div class="stars-outer">
                                <div class="stars-inner" style="width:{{ $SellerReview['ReviewPercentage'] }}%;">
                                </div>
                              </div>
                ...
                <div class="item-card">
                  <div class="item-image">
                    ...
                    <a href="{{ route('frontend.product', [$row->id, $row->slug]) }}"><img
                           src="{{ asset('media/' . $row->f_thumbnail) }}" alt="{{ $row->title }}" /></a>
                  </div>
...
@push('scripts')
  <script type="text/javascript">
    ...
    TEXT['The value must be less than or equal to'] =
      "{{ __('The value must be less than or equal to') }} {{ $data->is_stock == 1 ? $data->stock_qty : '' }}";
