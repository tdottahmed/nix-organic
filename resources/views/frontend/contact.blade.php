...
@section('content')
  ...
  <main class="main">
    ...
    @if ($data['is_publish'] == 1)
      ...
      <section class="inner-section inner-section-bg contact_card">
        <div class="container">
          <div class="row">
                      ...
                      <div class="row">
                        <div class="col-md-12">
                          <div class="form-group">
                            ...
                            @endif
                            @if ($row->type == 'text')
                              <input type="text" name="{{ str_slug($row->name) }}" id="{{ str_slug($row->name) }}"
                                     placeholder="{{ $row->placeholder }}"
                                     class="form-control {{ $row->mandatory == 'yes' ? 'parsley-validated' : '' }}"
                                     {{ $row->mandatory == 'yes' ? 'data-required="true"' : '' }}>
                            @elseif($row->type == 'email')
                              <input type="email" name="{{ str_slug($row->name) }}" id="{{ str_slug($row->name) }}"
                                     placeholder="{{ $row->placeholder }}"
                                     class="form-control {{ $row->mandatory == 'yes' ? 'parsley-validated' : '' }}"
                                     {{ $row->mandatory == 'yes' ? 'data-required="true"' : '' }}>
                            @elseif($row->type == 'dropdown')
                              <select name="{{ str_slug($row->name) }}" id="{{ str_slug($row->name) }}"
                                      class="chosen-select form-control {{ $row->mandatory == 'yes' ? 'parsley-validated' : '' }}"
                                      {{ $row->mandatory == 'yes' ? 'data-required="true"' : '' }}>
                            ...
                            @elseif($row->type == 'textarea')
                              <textarea name="{{ str_slug($row->name) }}" id="{{ str_slug($row->name) }}" placeholder="{{ $row->placeholder }}"
                                        class="form-control {{ $row->mandatory == 'yes' ? 'parsley-validated' : '' }}"
            ...
            <div class="col-lg-5">
              <div class="contact-info">
                <h3 class="heading">{{ __('Contact Info') }}</h3>
