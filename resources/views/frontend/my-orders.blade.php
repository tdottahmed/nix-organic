                          ...
                          @foreach ($datalist as $row)
                            ...
                            <tr>
                              <td class="text-left"><a
                                   href="{{ route('frontend.order-details', [$row->id, $row->order_no]) }}">{{ $row->order_no }}</a>
                              </td>
                              <td class="text-left">{{ date('d-m-Y', strtotime($row->created_at)) }}</td>

                              @if ($gtext['currency_position'] == 'left')
                                <td class="text-center">{{ $gtext['currency_icon'] }}{{ NumberFormat($total_amount) }}
                                </td>
                              @else
                                <td class="text-center">{{ NumberFormat($total_amount) }}{{ $gtext['currency_icon'] }}
                                </td>
                              @endif
                              ...
                              <td class="text-center"><span
                                      class="status_btn pstatus_{{ $row->payment_status_id }}">{{ $row->pstatus_name }}</span>
                              </td>
                              <td class="text-center"><span
                                      class="status_btn ostatus_{{ $row->order_status_id }}">{{ $row->ostatus_name }}</span>
                              </td>

                              <td class="text-center">
                                <a title="{{ __('Invoice') }}" class="mr10"
                                   href="{{ route('frontend.order-invoice', [$row->id, $row->order_no]) }}"><i
                                     class="bi bi-cloud-arrow-down"></i></a>
                                <a title="{{ __('View') }}"
                                   href="{{ route('frontend.order-details', [$row->id, $row->order_no]) }}"><i
                                     class="bi bi-eye"></i></a>
                              </td>
                            </tr>
                          @endforeach
