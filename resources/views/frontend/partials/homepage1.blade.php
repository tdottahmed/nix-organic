<main class="main {{ $PageVariation['home_variation'] }}">
  ...
  @if ($section1->is_publish == 1)
    <section class="slider-section">
      <div class="slider-screen h1-height"
           style="background-image: url({{ $section1->image ? asset('media/' . $section1->image) : '' }});">
        <div class="home-slider owl-carousel">
          @foreach ($slider as $row)
            ...
            <div class="single-slider">
              <div class="container">
                <div class="row">
                  <div class="col-sm-12 order-sm-1 col-md-6 order-md-0 col-lg-5 order-lg-0 order-1">
                    <div class="slider-content">
                      ...
                      @if ($aRow->button_text != '')
                        <a href="{{ $row->url }}" class="btn theme-btn"
              ...
              <div class="item-card">
                ...
                <div class="item-card-bottom">
                  ...
                  <ul class="item-cart-list">
                    ...
                    <li><a href="{{ route('frontend.product', [$row->id, $row->slug]) }}"><i class="bi bi-eye"></i></a>
                    </li>
                  </ul>
                </div>
              </div>
  ...
  @if ($section4->is_publish == 1)
    @if (count($offer_ad_position1) > 0)
      <section class="section offer-section"
               style="background-image: url({{ $section4->image ? asset('media/' . $section4->image) : '' }});">
        <div class="container">
          <div class="row">
            <div class="col-md-12">
              <div class="section-heading">
            ...
            <ul class="nav nav-pills tp-tabs" id="pills-tab" role="tablist">
              @if ($section8->is_publish == 1)
                <li class="nav-item" role="presentation">
                  <button class="nav-link active" id="pills-newProducts-tab" data-bs-toggle="pill"
                          data-bs-target="#pills-newProducts" type="button" role="tab"
                          aria-controls="pills-newProducts" aria-selected="true">{{ $section8->title }}</button>
                </li>
              @endif
              @if ($section9->is_publish == 1)
                <li class="nav-item" role="presentation">
                  <button class="nav-link" id="pills-topSelling-tab" data-bs-toggle="pill"
                          data-bs-target="#pills-topSelling" type="button" role="tab"
                          aria-controls="pills-topSelling" aria-selected="false">{{ $section9->title }}</button>
                </li>
              @endif
              @if ($section10->is_publish == 1)
                <li class="nav-item" role="presentation">
                  <button class="nav-link" id="pills-trendingProducts-tab" data-bs-toggle="pill"
                          data-bs-target="#pills-trendingProducts" type="button" role="tab"
                          aria-controls="pills-trendingProducts"
                          aria-selected="false">{{ $section10->title }}</button>
                </li>
              @endif
              @if ($section11->is_publish == 1)
                <li class="nav-item" role="presentation">
                  <button class="nav-link" id="pills-topRated-tab" data-bs-toggle="pill"
                          ...
                          aria-controls="pills-topRated" aria-selected="false">{{ $section11->title }}</button>
                </li>
              @endif
            </ul>
          </div>
        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="tab-content" id="pills-tabContent">
              <!--New Products-->
              @if ($section8->is_publish == 1)
                <div class="tab-pane fade show active" id="pills-newProducts" role="tabpanel"
                     aria-labelledby="pills-newProducts-tab">
                  <div class="row">
                        ...
                        <div class="item-card">
                            ...
                            @if ($row->is_discount == 1 && $row->old_price != '')
                              @php
                                $discount = number_format(
                                    (($row->old_price - $row->sale_price) * 100) / $row->old_price,
                                );
                              @endphp
                              <span class="item-label">{{ $discount }}% {{ __('Off') }}</span>
                            @endif
                          ...
                          <div class="item-pric-card">
                            @if ($row->sale_price != '')
                              @if ($gtext['currency_position'] == 'left')
                                <div class="new-price">
                                  {{ $gtext['currency_icon'] }}{{ NumberFormat($row->sale_price) }}</div>
                              @else
                                <div class="new-price">
                                  {{ NumberFormat($row->sale_price) }}{{ $gtext['currency_icon'] }}</div>
                              @endif
                            @endif
                            @if ($row->is_discount == 1 && $row->old_price != '')
                              @if ($gtext['currency_position'] == 'left')
                                <div class="old-price">
                                  {{ $gtext['currency_icon'] }}{{ NumberFormat($row->old_price) }}</div>
                              @else
                                <div class="old-price">
                                  {{ NumberFormat($row->old_price) }}{{ $gtext['currency_icon'] }}</div>
                              @endif
                            @endif
                          </div>
                          <div class="item-card-bottom">
                            <a data-id="{{ $row->id }}" href="javascript:void(0);"
                               class="btn add-to-cart addtocart">{{ __('Add To Cart') }}</a>
                            <ul class="item-cart-list">
                              <li><a class="addtowishlist" data-id="{{ $row->id }}"
                                   href="javascript:void(0);"><i class="bi bi-heart"></i></a></li>
              ...
              <!--Top Selling-->
              @if ($section9->is_publish == 1)
                <div class="tab-pane fade" id="pills-topSelling" role="tabpanel"
                     aria-labelledby="pills-topSelling-tab">
                  <div class="row">
              ...
              <!--Trending Products-->
              @if ($section10->is_publish == 1)
                <div class="tab-pane fade" id="pills-trendingProducts" role="tabpanel"
                     aria-labelledby="pills-trendingProducts-tab">
                  <div class="row">
  ...
  @if ($home_video['is_publish'] == 1)
    <section class="section video-section"
             style="background-image: url({{ asset('media/' . $home_video['image']) }});">
      <div class="container">
          ...
          <div class="col-xl-5">
            <div class="video-desc">
              ...
              <a href="{{ $home_video['url'] }}"
                 ...
                 class="btn theme-btn">{{ $home_video['button_text'] }}</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="container">
        ...
        <div class="row">
          @if (count($offer_ad_position2) > 0)
            <div class="col-sm-12 order-sm-1 col-md-4 order-md-1 col-lg-3 order-lg-0 order-1">
              @foreach ($offer_ad_position2 as $row)
                @php $aRow = json_decode($row->desc); @endphp
                <div class="deals-card"
                     style="background:{{ $aRow->bg_color == '' ? '#d7eabf' : $aRow->bg_color }};">
                  <img src="{{ asset('media/' . $row->image) }}" alt="{{ $aRow->text_1 }}" />
                  @if ($aRow->text_1 != '')
              ...
              <div class="col-lg-12">
                <div class="item-card">
                  <div class="item-image">
                    ...
                    @if ($row->is_discount == 1 && $row->end_date != '')
                      <div class="deals-countdown-card">
                        <div data-countdown="{{ date('Y/m/d', strtotime($row->end_date)) }}"
                             class="deals-countdown"></div>
                      </div>
                    @endif
                  </div>
                  <div class="item-title">
                  ...
                  <div class="item-pric-card">
                    @if ($row->sale_price != '')
                      @if ($gtext['currency_position'] == 'left')
                        <div class="new-price">{{ $gtext['currency_icon'] }}{{ NumberFormat($row->sale_price) }}
                        </div>
                      @else
                        <div class="new-price">{{ NumberFormat($row->sale_price) }}{{ $gtext['currency_icon'] }}
                        </div>
                      @endif
                    @endif
                    @if ($row->is_discount == 1 && $row->old_price != '')
                      @if ($gtext['currency_position'] == 'left')
                        <div class="old-price">{{ $gtext['currency_icon'] }}{{ NumberFormat($row->old_price) }}
                        </div>
                      @else
                        <div class="old-price">{{ NumberFormat($row->old_price) }}{{ $gtext['currency_icon'] }}
                        </div>
                      @endif
                    @endif
                  </div>
