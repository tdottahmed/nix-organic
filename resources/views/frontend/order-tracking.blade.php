      ...
      <div class="container">
            ...
            <div class="register shadow_bg_off">
              ...
              <form class="form" method="GET" action="{{ route('frontend.order-tracking') }}">
                <div class="form-group">
                  <input name="order_no" type="text" class="form-control" placeholder="{{ __('Order#') }}"
                         required />
                </div>
                <div class="form-group">
                  <input name="email" type="email" class="form-control" placeholder="{{ __('Email Address') }}"
                         required />
                </div>
        ...
        <div class="my_card mb40">
                ...
                <div class="col-lg-12">
                  <div class="order_tracking_card">
                    ...
                    <ul class="order_track">
                      <li>
                        <span class="order_track_item {{ $awaiting }}"><i
                             class="bi bi-{{ $awaiting }}"></i></span>
                        <strong>{{ $order_status_list[1] }}</strong>
                      </li>
                      <li>
                        <span class="order_track_item {{ $processing }}"><i
                             class="bi bi-{{ $processing }}"></i></span>
                        <strong>{{ $order_status_list[2] }}</strong>
                      </li>
                      <li>
                        <span class="order_track_item {{ $pickup }}"><i
                             class="bi bi-{{ $pickup }}"></i></span>
                        <strong>{{ $order_status_list[3] }}</strong>
                      </li>
                      <li>
                        <span class="order_track_item {{ $completed }}"><i
                             class="bi bi-{{ $completed }}"></i></span>
                ...
                @foreach ($masterData as $mdata)
                  <div class="row mb10">
                    ...
                    <div class="col-lg-6 mb10 order_status">
                      ...
                      <p class="mb5"><strong>{{ __('Order Date') }}</strong>:
                        {{ date('d-m-Y', strtotime($mdata->created_at)) }}</p>
                      <p class="mb5"><strong>{{ __('Payment Method') }}</strong>: {{ $mdata->method_name }}</p>
                      <p class="mb5"><strong>{{ __('Payment Status') }}</strong>: <span
                              class="status_btn pstatus_{{ $mdata->payment_status_id }}">{{ $mdata->pstatus_name }}</span>
                      </p>
                      <p class="mb5"><strong>{{ __('Order Status') }}</strong>: <span
                              class="status_btn ostatus_{{ $mdata->order_status_id }}">{{ $mdata->ostatus_name }}</span>
                      </p>
                    </div>
                  </div>
                @endforeach
                            ...
                            <tr>
                              ...
                              <td class="pro-name-w">
                                <span class="pro-name"><a
                                     href="{{ route('frontend.product', [$row->id, str_slug($row->title)]) }}">{{ $row->title }}</a></span>
                              </td>
                        ...
                        <table class="table">
                          <tbody>
                            @foreach ($masterData as $mdata)
                              ...
                              <tr>
                                <td><span
                                        class="title">{{ __('Shipping Fee') }}<br>({{ $mdata->shipping_title }})</span><span
                                        class="price">{{ $shipping_fee }}</span></td>
                              </tr>
                              <tr>
                              ...
                              <tr>
                                <td><span class="title">{{ __('Subtotal') }}</span><span
                                        class="price">{{ $subtotal }}</span></td>
                              </tr>
                              <tr>
                                <td><span class="total">{{ __('Total') }}</span><span
                                        class="total-price">{{ $total_amount }}</span></td>
                              </tr>
                            @endforeach
                          </tbody>
                        </table>
          ...
          @else
            <div class="row">
              <div class="col-lg-12">
                <h5 class="red text-center" style="display: {{ $isfind }};">{{ __('No data available') }}</h5>
              </div>
            </div>
          @endif
        </div>
