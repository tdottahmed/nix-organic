          ...
          <div class="col-lg-12">
            <div class="card">
                ...
                <div class="row">
                  ...
                  <div class="col-lg-6">
                    <div class="group-button float-right">
                      <button type="button" onClick="onExcelExport()" class="btn btn-theme mb0 btn-padding"><i
                           class="fa fa-download"></i> {{ __('Excel') }}</button>
                      <button type="button" onClick="onCSVExport()" class="btn btn-theme mb0 btn-padding"><i
                           class="fa fa-download"></i> {{ __('CSV') }}</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="card-body">
                  ...
                  <div class="col-lg-12">
                    <div class="group-button">
                      <button id="orderstatus_0" type="button" onClick="onDataViewByStatus(0)"
                              class="btn btn-theme orderstatus active">{{ __('All') }} (@php echo OrderCountForSeller(0); @endphp)</button>
                      @foreach ($order_status_list as $row)
                        <button id="orderstatus_{{ $row->id }}" type="button"
                                onClick="onDataViewByStatus({{ $row->id }})"
                                class="btn btn-theme orderstatus">{{ $row->ostatus_name }} (@php echo OrderCountForSeller($row->id); @endphp)</button>
                      @endforeach
                    </div>
                ...
                <div id="tp_datalist">
                  @include('seller.partials.orders_table')
                </div>
              </div>
              <!--/Data grid/-->
            </div>
...
@push('scripts')
  ...
  <script src="{{ asset('backend/pages/seller-orders.js') }}"></script>
@endpush
